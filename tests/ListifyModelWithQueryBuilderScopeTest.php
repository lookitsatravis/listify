...
class ListifyModelWithQueryBuilderScopeTest extends ListifyBaseTest {
    ...
    public function test_passScopeInWithMissingWhere()
    {
        ...
        $foo->setListifyConfig('scope', Capsule::table('foos')->orderBy('id ASC'));
        $foo->save();
    }
    ...
    public function test_changeScopeBeforeUpdate()
    {
        ...
        $foo1->setListifyConfig('scope', Capsule::table('foo_with_query_builder_scopes')->where('company', '=', 'TestCompany1'));
        $foo1->save();

        $foo2 = new $this->model;
        $foo2->name = $this->model . "Test2";
        $foo2->company = 'TestCompany1';
        $foo2->setListifyConfig('scope', Capsule::table('foo_with_query_builder_scopes')->where('company', '=', 'TestCompany1'));
        $foo2->save();

        Assert::eq(1, $foo1->getListifyPosition());
        Assert::eq(2, $foo2->getListifyPosition());

        $foo1->setListifyConfig('scope', Capsule::table('foo_with_query_builder_scopes')->where('company', '=', 'TestCompany2'));
        $foo1->save();
